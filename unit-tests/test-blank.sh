...
if printf "${test_content}" > "${__UNIT_TESTS_DIR}/test_blank_file.txt" 2>/dev/null; then
...
if touch "${__UNIT_TESTS_DIR}/test_blank_empty.txt" 2>/dev/null; then
...
if printf "Some content\nMore content\n" > "${__UNIT_TESTS_DIR}/test_blank_basic.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_basic.txt" 2>/dev/null; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_basic.txt" ]]; then
...
if printf "Test content\n" > "${__UNIT_TESTS_DIR}/test_blank_success.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_success.txt" 2>&1 | grep -q "Successfully emptied"; then
...
if printf "Test content\n" > "${__UNIT_TESTS_DIR}/test_blank_return.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_return.txt" >/dev/null 2>&1; then
        if [[ $? -eq 0 ]]; then
...
if touch "${__UNIT_TESTS_DIR}/test_blank_already_empty.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_already_empty.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_already_empty.txt" ]]; then
...
if [[ ! -f "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" ]]; then
    if blank --touch -x "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" ]] && [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" ]]; then
    ...
    rm -f "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt"
    if blank --touch -x "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" ]] && [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_touch_new.txt" ]]; then
            if print_msg 17 "Does --touch create new file and empty it?" true; then
                ((score++))
...
if rm -f "${__UNIT_TESTS_DIR}/test_blank_touch_warning.txt" 2>/dev/null; then
    if blank --touch -x "${__UNIT_TESTS_DIR}/test_blank_touch_warning.txt" 2>&1 | grep -q "Warning.*creating new file"; then
...
if rm -f "${__UNIT_TESTS_DIR}/test_blank_t_short.txt" 2>/dev/null; then
    if blank -t -x "${__UNIT_TESTS_DIR}/test_blank_t_short.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_t_short.txt" ]]; then
...
if printf "Existing content\n" > "${__UNIT_TESTS_DIR}/test_blank_touch_existing.txt" 2>/dev/null; then
    if ! blank --touch -x "${__UNIT_TESTS_DIR}/test_blank_touch_existing.txt" 2>&1 | grep -q "Warning.*creating new file"; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_touch_existing.txt" ]]; then
...
if printf "Content to be emptied\n" > "${__UNIT_TESTS_DIR}/test_blank_x_flag.txt" 2>/dev/null; then
    start_time=$(date +%s)
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_x_flag.txt" >/dev/null 2>&1; then
        end_time=$(date +%s)
        elapsed=$((end_time - start_time))
        if [[ $elapsed -lt 2 ]] && [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_x_flag.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_nocountdown.txt" 2>/dev/null; then
    if blank --no-countdown "${__UNIT_TESTS_DIR}/test_blank_nocountdown.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_nocountdown.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_skipcountdown.txt" 2>/dev/null; then
    if blank --skip-countdown "${__UNIT_TESTS_DIR}/test_blank_skipcountdown.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_skipcountdown.txt" ]]; then
...
if rm -f "${__UNIT_TESTS_DIR}/test_blank_comb1.txt" 2>/dev/null; then
    if blank --touch -x "${__UNIT_TESTS_DIR}/test_blank_comb1.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_comb1.txt" ]] && [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_comb1.txt" ]]; then
...
if rm -f "${__UNIT_TESTS_DIR}/test_blank_comb2.txt" 2>/dev/null; then
    if blank -x --touch "${__UNIT_TESTS_DIR}/test_blank_comb2.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_comb2.txt" ]] && [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_comb2.txt" ]]; then
...
if rm -f "${__UNIT_TESTS_DIR}/test_blank_comb3.txt" 2>/dev/null; then
    if blank --touch "${__UNIT_TESTS_DIR}/test_blank_comb3.txt" -x >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_comb3.txt" ]] && [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_comb3.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test blank file with spaces.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test blank file with spaces.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test blank file with spaces.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test-blank-file-special@123.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test-blank-file-special@123.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test-blank-file-special@123.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_relative.txt" 2>/dev/null; then
    if blank -x "test_blank_relative.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_relative.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_absolute.txt" 2>/dev/null; then
    abs_path="${__UNIT_TESTS_DIR}/test_blank_absolute.txt"
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_multiple.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_multiple.txt" >/dev/null 2>&1 && \
       blank -x "${__UNIT_TESTS_DIR}/test_blank_multiple.txt" >/dev/null 2>&1 && \
       blank -x "${__UNIT_TESTS_DIR}/test_blank_multiple.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_multiple.txt" ]]; then
...
if dd if=/dev/urandom of="${__UNIT_TESTS_DIR}/test_blank_large.txt" bs=1K count=100 >/dev/null 2>&1; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_large.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_large.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_perms.txt" 2>/dev/null; then
    orig_perms=$(stat -c "%a" "${__UNIT_TESTS_DIR}/test_blank_perms.txt" 2>/dev/null || stat -f "%OLp" "${__UNIT_TESTS_DIR}/test_blank_perms.txt" 2>/dev/null || echo "")
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_perms.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_perms.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_arg1.txt" 2>/dev/null && \
   printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_arg2.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_arg1.txt" "${__UNIT_TESTS_DIR}/test_blank_arg2.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_arg1.txt" ]] && [[ -s "${__UNIT_TESTS_DIR}/test_blank_arg2.txt" ]]; then
...
if rm -f "${__UNIT_TESTS_DIR}/test_blank_case.txt" 2>/dev/null; then
    # Note: case-insensitive check depends on implementation - testing lowercase behavior
    if blank --touch -x "${__UNIT_TESTS_DIR}/test_blank_case.txt" >/dev/null 2>&1; then
        if [[ -f "${__UNIT_TESTS_DIR}/test_blank_case.txt" ]]; then
...
if printf "Content\n" > "${__UNIT_TESTS_DIR}/test_blank_noninteractive.txt" 2>/dev/null; then
    # Run in non-interactive context (piped input simulates non-interactive)
    if echo "" | blank "${__UNIT_TESTS_DIR}/test_blank_noninteractive.txt" >/dev/null 2>&1; then
        if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_noninteractive.txt" ]]; then
        ...
        if blank -x "${__UNIT_TESTS_DIR}/test_blank_noninteractive.txt" >/dev/null 2>&1; then
            if [[ ! -s "${__UNIT_TESTS_DIR}/test_blank_noninteractive.txt" ]]; then
                if print_msg 36 "Does blank work in non-interactive context?" true; then
                    ((score++))
                    if type update_progress_from_score >/dev/null 2>&1; then
...
if printf "Original content\n" > "${__UNIT_TESTS_DIR}/test_blank_after_write.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_after_write.txt" >/dev/null 2>&1; then
        if printf "New content after blanking\n" > "${__UNIT_TESTS_DIR}/test_blank_after_write.txt" 2>/dev/null; then
            if [[ -s "${__UNIT_TESTS_DIR}/test_blank_after_write.txt" ]] && \
               grep -q "New content after blanking" "${__UNIT_TESTS_DIR}/test_blank_after_write.txt" 2>/dev/null; then
...
if printf "Multi-line content\nLine 2\nLine 3\nEnd\n" > "${__UNIT_TESTS_DIR}/test_blank_size.txt" 2>/dev/null; then
    if blank -x "${__UNIT_TESTS_DIR}/test_blank_size.txt" >/dev/null 2>&1; then
        file_size=$(stat -c "%s" "${__UNIT_TESTS_DIR}/test_blank_size.txt" 2>/dev/null || stat -f "%z" "${__UNIT_TESTS_DIR}/test_blank_size.txt" 2>/dev/null || echo "-1")
...
rm -f "${__UNIT_TESTS_DIR}"/test_blank_*.txt "${__UNIT_TESTS_DIR}"/test-blank-*.txt 2>/dev/null
rm -f "${__UNIT_TESTS_DIR}/test blank file with spaces.txt" 2>/dev/null
